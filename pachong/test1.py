...
proxy_ips = ['122.226.189.55']  # 替换自己的代理IP
headers = {
    ...
    def spider(self, vol):
        ...
        for track in track_names:
            _id = str(int(track.text[:2])) if (int(vol) < 12) else track.text[
                                                                   :2]  # 12期前的音乐编号1~9是1位（如：1~9）
            # ，之后的都是2位 1~9会在左边垫0（如：01~09）
...
if __name__ == '__main__':
    ...
    for i in range(downloader_count):
        luoo_download = LuooDownloader(luoo_site_mp3, 'C:/Users/ASUS/untitled2/kk',
                                       queue=spider_queue)
